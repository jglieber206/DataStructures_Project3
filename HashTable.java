...
public class HashTable {
	/**
	 * for now the table size is defined
	 * however, we want to have an ArrayList instead of an array
	 */
	...
	public Object getValue(Object key)
	{
		...
		if (this.table[hashVal] != null) 
		{
			...
			HashtableNode node = new HashtableNode();
			node.setKey(key);
			int pos = ((LinkedList)this.table[hashVal]).indexOf(node);
			if ( pos > -1) 
			{
//				System.out.println("in if statement");
				HashtableNode value = (HashtableNode)((LinkedList)this.table[hashVal]).elementAt(pos);
				val = value.getData();
				return val;
			}
...
//		buffer.append(System.getProperty("line.separator"));
		...
		for (int i = 0; i < this.table.length; i++) 
		{
			...
			{
//				returnVal = returnVal + ((LinkedList)this.table[i]+"\n");
//				buffer.append(" ");
